...
Complete code samples for every feature of `@thaparoyal/calendar-vanilla`. All examples use ESM imports; for CDN usage replace imports with the `TRCalendar` global (see [CDN Usage](/docs/vanilla/cdn/)).
...
Each theme also has a `-dark` variant (e.g. `ocean-dark`). See [Themes](/docs/core/themes/) for details.
