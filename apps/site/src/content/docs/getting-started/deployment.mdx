...
1. **Trigger** — runs on push to `main` when files change in `apps/site/`, `packages/`, or the workflow itself
2. **Build** — installs dependencies, builds all packages (core + framework packages), then builds the Astro site
...
```bash
# Build everything (packages + site)
pnpm build

# Or just the site (if packages are already built)
pnpm build:site
...
The site imports from the local packages. Make sure `pnpm build` builds all packages before the site:

```bash
# This builds packages first, then site (configured in turbo.json)
pnpm build
```
